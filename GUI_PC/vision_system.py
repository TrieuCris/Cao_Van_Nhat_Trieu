    ...
    def _process_frame(self, frame, capture_time, run_ai=True):
        ...
        if self.camera_config:
            ...
            for obj in tracked_objects:
                ...
                if status == "scheduled": box_color = (0, 255, 0)
                elif status == "discarded": box_color = (0, 0, 255)
                ...
                # Show coordinates (Robot coords if calibrated, else Pixel coords)
                if self.camera_config and self.camera_config.is_calibrated():
                    robot_pos = self.camera_config.pixel_to_mm(cx, cy)
                    if robot_pos:
                        label += f" ({robot_pos[0]:.0f}, {robot_pos[1]:.0f})"
                else:
                    label += f" ({int(cx)}, {int(cy)})"

                cv2.putText(frame, label, (new_cx-30, new_cy-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, box_color, 1)
