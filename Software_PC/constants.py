...
# [UPDATE] Tăng tốc độ JOG 70 -> 120 RPM (theo yêu cầu nhanh hơn)
JOG_SPEED_RPM = 60   # Tốc độ JOG 
...
# [UPDATE] Tăng vận tốc tối đa 500 -> 800 mm/s
TRAJ_LINEAR_V_MAX_MM_S = 500.0   # Tốc độ chạy ổn định (mm/s)

# ✅ FIX GIẬT: Giảm gia tốc để khởi động mềm, đặc biệt tại vị trí HOME (gần singularity)
# Lý do: Độ rơ hộp số + vận tốc góc cao tại HOME → cần tăng tốc từ từ
TRAJ_LINEAR_ACCEL_MM_S2 = 2500.0 # Gia tốc (mm/s^2) - Giảm từ 3000 xuống 1500
TRAJ_LINEAR_V_START_MM_S = 50.0  # Tốc độ bắt đầu (mm/s) - Tăng từ 0 lên 50 để "vượt rơ" mượt hơn
...
# ⚠️ REVERTED: 10ms gây quá tải STM32 (100 Hz quá nhanh cho USB Serial)
# Giữ 20ms (50 Hz) để cân bằng giữa độ mượt và độ tin cậy
TRAJ_SEGMENT_TIME = 0.040  # 20ms (50 Hz)
...
# [UPDATE] Tăng 0.05 -> 0.5 để cua nhanh hơn, robot không bị khựng lại ở các waypoint
TRAJ_JUNCTION_DEVIATION = 0.5
